...
用法（在容器内跑，依赖 pandas/pyarrow + 交易日历）：
    docker exec market-data sh -lc "cd /app && python scripts/scan_cache_integrity.py --top 20"
    docker exec market-data sh -lc "cd /app && python scripts/scan_cache_integrity.py --namespace fund_share --top 30"
