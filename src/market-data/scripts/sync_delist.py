...
用法（容器内执行，`src/marketdata` 以 bind mount 挂在 /app）
------------------------------------------------------------------
    docker exec market-data python scripts/sync_delist.py              # 增量
    docker exec market-data python scripts/sync_delist.py --full        # 全量回填
    docker exec market-data python scripts/sync_delist.py --market SH   # 只跑沪市
    docker exec market-data python scripts/sync_delist.py --query 560650
